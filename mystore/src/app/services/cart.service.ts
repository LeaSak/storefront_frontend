...
export class CartService {
  ...
  removeItem(product: ProductOrder){
    const item = this.checkItem(product);
    if (item){
      if (item.quantity > 1){
        item.quantity -= 1;
  ...
  calcCartLength(): number {
    const quantity = this.cart.reduce(function(currentValue, item: ProductOrder){
  ...
  checkItem(product: Product): ProductOrder | undefined {
    const item = this.cart.find((cartItem: Product) => cartItem.id === product.id);
    return item;
  }

  setOrderQuantity(item: ProductOrder){
  ...
  setTotalPrice(item: ProductOrder){
    item.totalPrice = this.calcTotalPrice(item);
  }

  getTotalPrice(item: ProductOrder){
    return item.totalPrice;
  }

  calcOrderTotal(): number{
    const summary = this.cart.reduce(function(currentValue, item: ProductOrder){
